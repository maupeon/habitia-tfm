...
Los cálculos float32 pueden variar en sus últimos bits según el procesador y las bibliotecas numéricas. Solo las dos aserciones contra importes guardados usan tolerancia relativa `2e-7`; una comprobación entre entornos detectó una diferencia de una unidad de precisión (ULP) del precio base, propagada por los factores del paquete. La igualdad exacta con la referencia se verifica dentro del mismo proceso y entorno, y las comprobaciones aritméticas de compra y alquiler se mantienen sin relajar.
...
Se integró el paquete `habitia_predictor`: XGBoost de 410 árboles, MdAPE histórico declarado 9,299045 % y RMSE log 0,185610. Sus pesos y métricas son idénticos a los de la versión 3.2 de la integración; cambian las tablas de índices y variables de barrio y los metadatos. Los seis artefactos conservan exactamente los bytes recibidos y sus hashes se comprueban antes de cargar el modelo. El manifiesto identifica la exportación original, y `source_code_sha256` identifica sus cinco módulos, incluido `rutas.py`.
